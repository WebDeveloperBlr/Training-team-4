...
exports.all = function(limit,filter,cb){
  var data ={};
  var query = 'SELECT position.name \'position\', vacancy.requirements, vacancy.workExperience,vacancy.salary\n' +
    'FROM `hr-app`.vacancy \n' +
    'INNER JOIN `hr-app`.vacancyPosition ON vacancyPosition.id_vacancy=vacancy.id_vacancy \n' +
    'INNER JOIN position ON position.id_position=vacancyPosition.id_position '+
    'WHERE (' +
    'position.name LIKE \'%'+filter[0][0]+'%\'';
    if(filter[1][0]&&filter[1][1]){
    query+='AND vacancy.workExperience >= \''+filter[1][0]+'\'' +
      'AND vacancy.workExperience < \''+filter[1][1]+'\'';
  }
    if(filter[2][0]!==""){
